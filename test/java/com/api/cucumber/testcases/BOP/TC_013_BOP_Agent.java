...
//BOP LOB
public class TC_013_BOP_Agent extends ProjMethods {
	...
	@Given("^Login to the application as Agent and select transaction \"([^\"]*)\"$")
	...
	public void fillAccountInfo(@Transform(ExcelDataToDataTable.class) DataTable table) throws Throwable
	{
		...
		.clickNext()
		.clickOk()
		.selectBuisnessType(data.get(3).get(8).toString())
	...
	@And("^Select market segment,SIC Code for Agent \"([^\"]*)\"$")
